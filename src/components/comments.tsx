...
interface Comment { id: string; author: string; premium: boolean; text: string; ts: number; correction?: string }

const seed: Comment[] = [
  { id: "1", author: "Minji_22", premium: true, text: "Le narrateur reçoit un message d'un vieux numéro et son cœur s'emballe.", ts: Date.now() - 3600_000, correction: "« 심장이 뛰기 시작했다 » est très bien construit ✅" },
  { id: "2", author: "Loïc_FR", premium: false, text: "J'ai compris l'idée mais la nuance de « 는 » m'échappe encore.", ts: Date.now() - 7200_000 },
];
        ...
        <div className="flex items-center justify-between mt-2 gap-3">
          {user.premium ? (
            <span className="text-xs text-gold inline-flex items-center gap-1">
              <Sparkles className="h-3 w-3" /> Vous pouvez demander 1 correction de commentaire coréen / épisode.
            </span>
          ) : <span className="text-xs text-muted-foreground">Connectez-vous pour commenter.</span>}
          <Button
            size="sm"
            disabled={!text.trim()}
            onClick={() => {
              setList([{ id: crypto.randomUUID(), author: user.pseudo, premium: user.premium, text, ts: Date.now() }, ...list]);
              setText("");
            }}
          >Publier</Button>
        </div>
        ...
        {list.map((c) => (
          <li key={c.id} className="border-b border-border/60 pb-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              {c.premium && <Badge variant="outline" className="border-gold/60 text-gold text-[10px]">Premium</Badge>}
